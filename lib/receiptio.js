        ...
        if (net.isIP(dest)) {
            ...
            try {
                ...
                const rid = setInterval(async () => {
                    ...
                    if (bytesRead > 0) {
                        conn.emit('data', buffer.slice(0, bytesRead));
                    }
                }, 100);
...
const createPrint = options => {
    ...
    const params = parseOption(options);
    // transform
    if (params.d || /^png$/i.test(params.p) || params.i && puppeteer) {
        ...
        return new stream.Transform({
            ...
            async flush(callback) {
                ...
                this.push(cmd, params.d || /^<svg/.test(cmd) ? 'utf8' : 'binary');
                callback();
            }
        });
    }
...
const convertOption = params => {
    ...
    if (!/^(svg|png|escpos|sii|citizen|fit|impactb?|star(line|graphic)?|emustarline)$/.test(p)) {
...
const transform = async (receiptmd, printer) => {
    ...
    if (printer.command === 'png') {
        return puppeteer ? await rasterize(receiptmd, printer, 'binary') : '';
    }
